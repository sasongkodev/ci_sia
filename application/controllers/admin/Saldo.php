...
class Saldo extends CI_Controller {
	...
	public function index()
	{
		...
		$data['sub_judul'] = 'Saldo';
		$data['content'] = 'admin/V_saldo';
		$data['data'] = $this->saldo_model->get();
	...
	public function add()
	{
		...
		$simpan = $this->saldo_model->add($data);
		if ($simpan) {
		...
		redirect(site_url('admin/saldo'),'refresh');
	}

	public function del($id)
	{

		$del = $this->saldo_model->del($id);
		if ($del) {
			$this->session->set_flashdata('info',
				'<div class="alert alert-success alert-dismissible flat">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
	...
	{
		$data['data'] = $this->saldo_model->get($id);
		$data['rek'] = $this->akun_model->get();
		$this->load->view('admin/V_edit_saldo', $data);
	}
